...
# Default-Stop:      0 1 6
# Short-Description: Start the fringe process
### END INIT INFO

# If you modify this, please make sure to also edit fringe.service

# Environment variables can be set in /etc/default/fringe. These will override
# any corresponding config file values.
DEFAULT=/etc/default/fringe
...
# Logging
if [ -z "$STDOUT" ]; then
    STDOUT=/var/log/fringe/fringe.log
fi
...
if [ -z "$STDERR" ]; then
    STDERR=/var/log/fringe/fringe.log
fi
...
function start() {
    ...
    if command -v start-stop-daemon &>/dev/null; then
        start-stop-daemon \
            ...
            --exec $DAEMON \
            -- \
            $FRINGE_OPTS >>$STDOUT 2>>$STDERR &
    else
        local CMD="$DAEMON $FRINGE_OPTS >>$STDOUT 2>>$STDERR &"
        su -s /bin/sh -c "$CMD" $USER
    fi

    # Sleep to verify process is still up
    sleep 1
    echo $(pgrep -u $USER -f fringe) > $PIDFILE
    if [ -f $PIDFILE ]; then
        # PIDFILE exists
